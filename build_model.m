...
outcoord2 = [l/2,3*b/4]; % output coordinate
dist2 = vecnorm(MyMesh.nodes(:,1:2) - repmat(outcoord2,[MyMesh.nNodes,1]),2,2);
[~,outnode2] = min(dist2);
outdof2 = (outnode2-1)*MyAssembly.Mesh.nDOFPerNode+outdir;

outdofvec2 = sparse(outdof2,ones(size(outdof2)),1,MyMesh.nDOFs,1);
outdofvec2 = MyAssembly.constrain_vector(outdofvec2);
outdof2 = find(outdofvec2);
outdof  = [outdof outdof2];
